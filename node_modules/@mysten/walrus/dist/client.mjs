import { MAINNET_WALRUS_PACKAGE_CONFIG, TESTNET_WALRUS_PACKAGE_CONFIG, statusLifecycleRank } from "./constants.mjs";
import { Storage } from "./contracts/walrus/storage_resource.mjs";
import { Blob, addMetadata, insertOrUpdateMetadataPair, removeMetadataPair } from "./contracts/walrus/blob.mjs";
import { Metadata, _new } from "./contracts/walrus/metadata.mjs";
import { StakingInnerV1 } from "./contracts/walrus/staking_inner.mjs";
import { StakingPool } from "./contracts/walrus/staking_pool.mjs";
import { Staking } from "./contracts/walrus/staking.mjs";
import { SystemStateInnerV1 } from "./contracts/walrus/system_state_inner.mjs";
import { System, certifyBlob, deleteBlob, extendBlob, registerBlob, reserveSpace } from "./contracts/walrus/system.mjs";
import { BehindCurrentEpochError, BlobBlockedError, BlobNotCertifiedError, InconsistentBlobError, NoBlobMetadataReceivedError, NoBlobStatusReceivedError, NoVerifiedBlobStatusReceivedError, NotEnoughBlobConfirmationsError, NotEnoughSliversReceivedError, RetryableWalrusClientError, WalrusClientError } from "./error.mjs";
import { IntentType, StorageConfirmation, blobIdToInt } from "./utils/bcs.mjs";
import { BlobNotRegisteredError, LegallyUnavailableError, NotFoundError, UserAbortError } from "./storage-node/error.mjs";
import { StorageNodeClient } from "./storage-node/client.mjs";
import { encodedBlobLength, getShardIndicesByNodeId, getSourceSymbols, isAboveValidity, isQuorum, signersToBitmap, sliverPairIndexFromSecondarySliverIndex, storageUnitsFromSize, toPairIndex, toShardIndex } from "./utils/index.mjs";
import { SuiObjectDataLoader } from "./utils/object-loader.mjs";
import { shuffle, weightedShuffle } from "./utils/randomness.mjs";
import { getWasmBindings } from "./wasm.mjs";
import { UploadRelayClient } from "./upload-relay/client.mjs";
import { encodeQuilt, encodeQuiltPatchId, parseWalrusId } from "./utils/quilts.mjs";
import { QuiltFileReader } from "./files/readers/quilt-file.mjs";
import { QuiltReader } from "./files/readers/quilt.mjs";
import { BlobReader } from "./files/readers/blob.mjs";
...
function walrus({ packageConfig, name = "walrus", ...options } = {}) {
	return {
		name,
		register: (client) => {
			const walrusNetwork = client.network;
			if (walrusNetwork !== "mainnet" && walrusNetwork !== "testnet") throw new WalrusClientError("Walrus client only supports mainnet and testnet");
			return new WalrusClient(packageConfig ? {
				packageConfig,
				suiClient: client,
				...options
			} : {
...
var WalrusClient = class {
	...
	#readCommittee;
	#cache;
	#uploadRelayConfig = null;
	#uploadRelayClient = null;
	constructor(config) {
		this.readBlob = this.#retryOnPossibleEpochChange(this.#internalReadBlob);
		this.getSecondarySliver = this.#retryOnPossibleEpochChange(this.internalGetSecondarySliver);
		if (config.network && !config.packageConfig) {
			...
			switch (network) {
				...
				case "mainnet":
					...
					break;
				default: throw new WalrusClientError(`Unsupported network: ${network}`);
			}
		} else this.#packageConfig = config.packageConfig;
		this.#wasmUrl = config.wasmUrl;
		this.#uploadRelayConfig = config.uploadRelay ?? null;
		if (this.#uploadRelayConfig) this.#uploadRelayClient = new UploadRelayClient(this.#uploadRelayConfig);
	...
	#walType() {
		return this.#cache.read(["walType"], async () => {
			const toStake = (await this.#suiClient.core.getMoveFunction({
			...
			if (toStakeCoinType?.$kind !== "datatype") throw new WalrusClientError("WAL type not found");
			return normalizeStructTag(toStakeCoinType.datatype.typeName);
		});
	}
	#getPackageId() {
		return this.#cache.read(["getPackageId"], async () => {
			return parseStructTag((await this.#objectLoader.load(this.#packageConfig.systemObjectId)).type).address;
		});
	}
	...
	async systemState() {
		return await this.#objectLoader.loadFieldObject(this.#packageConfig.systemObjectId, {
			type: "u64",
	...
	async stakingState() {
		return this.#objectLoader.loadFieldObject(this.#packageConfig.stakingPoolId, {
			type: "u64",
			value: (await this.stakingObject()).version
		}, StakingInnerV1);
	}
	async #internalReadBlob({ blobId, signal }) {
		const systemState = await this.systemState();
		const numShards = systemState.committee.n_shards;
		const blobMetadata = await this.getBlobMetadata({
			blobId,
			signal
		});
		const slivers = await this.getSlivers({
			blobId,
			signal
		});
		const bindings = await this.#wasmBindings();
		const blobBytes = bindings.decodePrimarySlivers(blobId, numShards, blobMetadata.metadata.V1.unencoded_length, slivers);
		if (bindings.computeMetadata(systemState.committee.n_shards, blobBytes).blobId !== blobId) throw new InconsistentBlobError("The specified blob was encoded incorrectly.");
		return blobBytes;
	}
	async computeBlobMetadata({ bytes, numShards }) {
		let shardCount;
		if (typeof numShards === "number") shardCount = numShards;
		else shardCount = (await this.systemState()).committee.n_shards;
		const { blobId, rootHash, unencodedLength, encodingType } = (await this.#wasmBindings()).computeMetadata(shardCount, bytes);
		let sha256Hash;
		...
		return {
			...
			nonce,
			blobDigest: () => {
				if (!sha256Hash) sha256Hash = crypto.subtle.digest("SHA-256", bytes).then((hash) => new Uint8Array(hash));
				return sha256Hash;
			}
		};
	}
	async getBlobMetadata({ blobId, signal }) {
		const randomizedNodes = shuffle((await this.#getReadCommittee({
			blobId,
			signal
		})).nodes);
		const numShards = (await this.stakingState()).n_shards;
		...
		const metadataExecutors = randomizedNodes.map((node) => async () => {
			try {
				return await this.#storageNodeClient.getBlobMetadata({ blobId }, {
					nodeUrl: node.networkUrl,
					signal: signal ? AbortSignal.any([controller.signal, signal]) : controller.signal
				});
			} catch (error) {
				if (error instanceof NotFoundError) numNotFoundWeight += node.shardIndices.length;
				else if (error instanceof LegallyUnavailableError) numBlockedWeight += node.shardIndices.length;
				totalErrorCount += 1;
				throw error;
			}
		});
		try {
			return await metadataExecutors.shift()();
		} catch {
			const chunkedExecutors = chunk(metadataExecutors, Math.floor(metadataExecutors.length / this.#blobMetadataConcurrencyLimit));
			return await new Promise((resolve, reject) => {
				chunkedExecutors.forEach(async (executors) => {
					for (const executor of executors) try {
						...
						if (totalErrorCount === metadataExecutors.length) reject(new NoBlobMetadataReceivedError("No valid blob metadata could be retrieved from any storage node."));
					}
				});
			});
		}
	...
	async internalGetSecondarySliver({ blobId, index, signal }) {
		...
		if (!node) throw new Error(`No node found for shard index ${shardIndex}`);
	...
	async getSlivers({ blobId, signal }) {
		const randomizedNodes = weightedShuffle((await this.#getReadCommittee({
			blobId,
			signal
		})).nodes.map((node) => ({
			value: node,
		...
		const sliverPairIndices = randomizedNodes.flatMap((node) => node.shardIndices.map((shardIndex) => ({
			url: node.networkUrl,
		...
		return new Promise((resolve, reject) => {
			chunkedSliverPairIndices[0].forEach(async (_, colIndex) => {
				for (let rowIndex = 0; rowIndex < chunkedSliverPairIndices.length; rowIndex += 1) {
					...
					try {
						if (failedNodes.has(url)) throw new Error(`Skipping node at ${url} due to previous failure.`);
						...
						if (error instanceof NotFoundError) numNotFoundWeight += 1;
						else if (error instanceof LegallyUnavailableError) numBlockedWeight += 1;
						...
						if (slivers.length + remainingTasks < minSymbols) {
							const abortError = new NotEnoughSliversReceivedError(`Unable to retrieve enough slivers to decode blob ${blobId}.`);
							controller.abort(abortError);
							reject(abortError);
						}
					}
	...
	*/
	async getVerifiedBlobStatus({ blobId, signal }) {
		...
		const numShards = (await this.stakingState()).n_shards;
		const controller = new AbortController();
		const sortedStatuses = [...(await new Promise((resolve, reject) => {
			const results = [];
			let successWeight = 0;
			let numNotFoundWeight = 0;
			let settledCount = 0;
			committee.nodes.forEach(async (node) => {
				...
				try {
					const status = await this.#storageNodeClient.getBlobStatus({ blobId }, {
						nodeUrl: node.networkUrl,
						signal: signal ? AbortSignal.any([controller.signal, signal]) : controller.signal
					});
					...
					if (error instanceof NotFoundError) numNotFoundWeight += weight;
					else if (error instanceof UserAbortError) reject(error);
					...
					settledCount += 1;
					if (settledCount === committee.nodes.length) reject(new NoBlobStatusReceivedError("Not enough statuses were retrieved to achieve quorum."));
				}
			});
			...
			if (existing) existing.totalWeight += weight;
			else accumulator.set(key, {
				status,
				totalWeight: weight
			});
			return accumulator;
		}, /* @__PURE__ */ new Map()).values()].toSorted((a, b) => statusLifecycleRank[b.status.type] - statusLifecycleRank[a.status.type]);
		for (const value of sortedStatuses) if (isAboveValidity(value.totalWeight, numShards)) return value.status;
		throw new NoVerifiedBlobStatusReceivedError(`The blob status could not be verified for blob ${blobId},`);
	}
	async #getCertificationEpoch({ blobId, signal }) {
		...
		if (stakingState.epoch_state.$kind === "EpochChangeSync") {
			const status = await this.getVerifiedBlobStatus({
				blobId,
				signal
			});
			if (status.type === "nonexistent" || status.type === "invalid") throw new BlobNotCertifiedError(`The specified blob ${blobId} is ${status.type}.`);
			if (typeof status.initialCertifiedEpoch !== "number") throw new BlobNotCertifiedError(`The specified blob ${blobId} is not certified.`);
			if (status.initialCertifiedEpoch > currentEpoch) throw new BehindCurrentEpochError(`The client is at epoch ${currentEpoch} while the specified blob was certified at epoch ${status.initialCertifiedEpoch}.`);
	...
	*/
	async #getReadCommittee(options) {
		if (!this.#readCommittee) this.#readCommittee = this.#forceGetReadCommittee(options);
		return this.#readCommittee;
	}
	async #forceGetReadCommittee({ blobId, signal }) {
		const stakingState = await this.stakingState();
		const isTransitioning = stakingState.epoch_state.$kind === "EpochChangeSync";
		const certificationEpoch = await this.#getCertificationEpoch({
			blobId,
			signal
		});
		if (isTransitioning && certificationEpoch < stakingState.epoch) return await this.#getCommittee(stakingState.previous_committee);
	...
	*/
	async storageCost(size, epochs) {
		const encodedSize = encodedBlobLength(size, (await this.systemState()).committee.n_shards);
		return this.#storageCostFromEncodedSize(encodedSize, epochs);
	}
	async #storageCostFromEncodedSize(encodedSize, epochs) {
		const systemState = await this.systemState();
	...
	*/
	createStorage({ size, epochs, walCoin }) {
		return async (tx) => {
			...
			const encodedSize = encodedBlobLength(size, (await this.systemState()).committee.n_shards);
			const [{ storageCost }, walrusPackageId] = await Promise.all([this.storageCost(size, epochs), this.#getWalrusPackageId()]);
			return tx.add(this.#withWal(storageCost, walCoin ?? null, (coin, tx$1) => {
				return tx$1.add(reserveSpace({
	...
	#withWal(amount, source, fn) {
		return async (tx) => {
			const walType = await this.#walType();
			const coin = source ? tx.splitCoins(source, [amount])[0] : tx.add(coinWithBalance({
				balance: amount,
				type: walType
			}));
	...
	* ```
	*/
	createStorageTransaction({ transaction = new Transaction(), size, epochs, owner }) {
		transaction.transferObjects([this.createStorage({
			size,
			epochs
		})], owner);
		return transaction;
	...
	* ```
	*/
	async executeCreateStorageTransaction({ signer, ...options }) {
		...
		const { digest, effects } = await this.#executeTransaction(transaction, signer, "create storage");
		const createdObjectIds = effects?.changedObjects.filter((object) => object.idOperation === "Created").map((object) => object.objectId);
		const suiBlobObject = (await this.#suiClient.core.getObjects({
			objectIds: createdObjectIds,
			include: { content: true }
		})).objects.find((object) => !(object instanceof Error) && object.type === blobType);
		if (suiBlobObject instanceof Error || !suiBlobObject) throw new WalrusClientError(`Storage object not found in transaction effects for transaction (${digest})`);
		return {
			digest,
	...
	* ```
	*/
	registerBlob({ size, epochs, blobId, rootHash, deletable, walCoin, attributes }) {
		return async (tx) => {
			const { writeCost } = await this.storageCost(size, epochs);
			const walrusPackageId = await this.#getWalrusPackageId();
			return tx.add(this.#withWal(writeCost, walCoin ?? null, async (writeCoin, tx$1) => {
				const blob = tx$1.add(registerBlob({
					package: walrusPackageId,
					arguments: {
						self: tx$1.object(this.#packageConfig.systemObjectId),
				...
				if (attributes) tx$1.add(this.#writeBlobAttributesForRef({
					attributes,
	...
	addAuthPayload({ size, blobDigest, nonce }) {
		return async (transaction) => {
			...
			const authPayload = new Uint8Array(nonceDigest.byteLength + digest.byteLength + lengthBytes.byteLength);
	...
	#loadTipConfig() {
		return this.#cache.read(["upload-relay-tip-config"], async () => {
			if (!this.#uploadRelayConfig?.sendTip || !this.#uploadRelayClient) return null;
			if ("kind" in this.#uploadRelayConfig.sendTip) return this.#uploadRelayConfig.sendTip;
	...
	async calculateUploadRelayTip(options) {
		...
		if (max != null && amount > max) throw new WalrusClientError(`Tip amount (${amount}) exceeds the maximum allowed tip (${max})`);
		return amount;
	}
	sendUploadRelayTip({ size, blobDigest, nonce }) {
		return async (transaction) => {
			const tipConfig = await this.#loadTipConfig();
			if (tipConfig) {
				transaction.add(this.addAuthPayload({
					size,
					blobDigest,
					nonce
				}));
				...
				transaction.transferObjects([coinWithBalance({ balance: amount })], address);
			}
		};
	}
	/**
	...
	* ```
	*/
	registerBlobTransaction({ transaction = new Transaction(), ...options }) {
	...
	* ```
	*/
	async executeRegisterBlobTransaction({ signer, ...options }) {
		const transaction = this.registerBlobTransaction({
			...options,
			owner: options.owner ?? options.transaction?.getData().sender ?? signer.toSuiAddress()
		});
		const blobType = await this.getBlobType();
		const { digest, effects } = await this.#executeTransaction(transaction, signer, "register blob");
		const createdObjectIds = effects?.changedObjects.filter((object) => object.idOperation === "Created").map((object) => object.objectId);
		const suiBlobObject = (await this.#suiClient.core.getObjects({
			objectIds: createdObjectIds,
			include: { content: true }
		})).objects.find((object) => !(object instanceof Error) && object.type === blobType);
		if (suiBlobObject instanceof Error || !suiBlobObject) throw new WalrusClientError(`Blob object not found in transaction effects for transaction (${digest})`);
		return {
			digest,
			blob: Blob.parse(suiBlobObject.content)
		};
	}
	async #getCreatedBlob(digest) {
		const blobType = await this.getBlobType();
		const result = await this.#suiClient.core.waitForTransaction({
			digest,
			include: { effects: true }
		});
		const createdObjectIds = (result.Transaction ?? result.FailedTransaction).effects?.changedObjects.filter((object) => object.idOperation === "Created").map((object) => object.objectId);
		const suiBlobObject = (await this.#suiClient.core.getObjects({
			objectIds: createdObjectIds,
			include: { content: true }
		})).objects.find((object) => !(object instanceof Error) && object.type === blobType);
	...
	async certificateFromConfirmations({ confirmations, blobId, deletable, blobObjectId }) {
		const systemState = await this.systemState();
		const committee = await this.#getActiveCommittee();
		if (confirmations.length !== systemState.committee.members.length) throw new WalrusClientError("Invalid number of confirmations. Confirmations array must contain an entry for each node");
		const confirmationMessage = StorageConfirmation.serialize({
			intent: IntentType.BLOB_CERT_MSG,
			epoch: systemState.committee.epoch,
			messageContents: {
				blobId,
				blobType: deletable ? { Deletable: { objectId: blobObjectId } } : { Permanent: null }
			}
		}).toBase64();
		const bindings = await this.#wasmBindings();
		const verifySignature = bindings.getVerifySignature();
		const filteredConfirmations = confirmations.map((confirmation, index) => {
			return confirmation?.serializedMessage === confirmationMessage && verifySignature(confirmation, new Uint8Array(committee.nodes[index].info.public_key.bytes)) ? {
				index,
				...confirmation
			} : null;
		}).filter((confirmation) => confirmation !== null);
		if (!isQuorum(filteredConfirmations.length, systemState.committee.members.length)) throw new NotEnoughBlobConfirmationsError(`Too many invalid confirmations received for blob (${filteredConfirmations.length} of ${systemState.committee.members.length})`);
		return bindings.combineSignatures(filteredConfirmations, filteredConfirmations.map(({ index }) => index));
	}
	/**
	...
	* ```
	*/
	certifyBlobTransaction({ transaction = new Transaction(), ...options }) {
	...
	* ```
	*/
	async executeCertifyBlobTransaction({ signer, ...options }) {
	...
	* ```
	*/
	deleteBlobTransaction({ owner, blobObjectId, transaction = new Transaction() }) {
	...
	* ```
	*/
	async executeDeleteBlobTransaction({ signer, transaction = new Transaction(), blobObjectId }) {
		const { digest } = await this.#executeTransaction(this.deleteBlobTransaction({
			blobObjectId,
			transaction,
	...
	*/
	extendBlob({ blobObjectId, epochs, endEpoch, walCoin }) {
		return async (tx) => {
			...
			const { storageCost } = await this.#storageCostFromEncodedSize(Number(blob.storage.storage_size), numEpochs);
			const walrusPackageId = await this.#getWalrusPackageId();
			return tx.add(this.#withWal(storageCost, walCoin ?? null, async (coin, tx$1) => {
				tx$1.add(extendBlob({
					package: walrusPackageId,
					arguments: {
						self: this.#packageConfig.systemObjectId,
	...
	* ```
	*/
	async extendBlobTransaction({ transaction = new Transaction(), ...options }) {
	...
	* ```
	*/
	async executeExtendBlobTransaction({ signer, ...options }) {
		const { digest } = await this.#executeTransaction(await this.extendBlobTransaction(options), signer, "extend blob");
		return { digest };
	}
	async readBlobAttributes({ blobObjectId }) {
		...
		return Object.fromEntries(parsedMetadata.metadata.contents.map(({ key, value }) => [key, value]));
	}
	#writeBlobAttributesForRef({ attributes, existingAttributes, blob }) {
		return async (tx) => {
			const walrusPackageId = await this.#getWalrusPackageId();
			if (!existingAttributes) tx.add(addMetadata({
				package: walrusPackageId,
				arguments: {
					self: blob,
					metadata: _new({ package: walrusPackageId })
				}
			}));
			Object.keys(attributes).forEach((key) => {
				const value = attributes[key];
				if (value === null) {
					if (existingAttributes && key in existingAttributes) tx.add(removeMetadataPair({
						package: walrusPackageId,
						arguments: {
							self: blob,
							key
				...
				} else tx.add(insertOrUpdateMetadataPair({
					package: walrusPackageId,
					arguments: {
						self: blob,
						key,
	...
	*/
	writeBlobAttributes({ blobObject, blobObjectId, attributes }) {
		return async (tx) => {
			...
			tx.add(this.#writeBlobAttributesForRef({
				attributes,
				existingAttributes,
				blob
			}));
	...
	* ```
	*/
	async writeBlobAttributesTransaction({ transaction = new Transaction(), ...options }) {
	...
	* ```
	*/
	async executeWriteBlobAttributesTransaction({ signer, ...options }) {
		const { digest } = await this.#executeTransaction(await this.writeBlobAttributesTransaction(options), signer, "write blob attributes");
		return { digest };
	}
	/**
	...
	*/
	async writeMetadataToNode({ nodeIndex, blobId, metadata, signal }) {
		const node = (await this.#getActiveCommittee()).nodes[nodeIndex];
		return retry(() => this.#storageNodeClient.storeBlobMetadata({
			blobId,
			metadata
		}, {
			nodeUrl: node.networkUrl,
	...
	* ```
	*/
	async getStorageConfirmationFromNode({ nodeIndex, blobId, deletable, objectId, signal }) {
		const node = (await this.#getActiveCommittee()).nodes[nodeIndex];
		return (deletable ? await this.#storageNodeClient.getDeletableBlobConfirmation({
			blobId,
			objectId
		}, {
			nodeUrl: node.networkUrl,
			signal
		}) : await this.#storageNodeClient.getPermanentBlobConfirmation({ blobId }, {
			nodeUrl: node.networkUrl,
			signal
		}))?.success?.data?.signed ?? null;
	}
	/**
	...
	*/
	async encodeBlob(blob) {
		...
		const committee = await this.#getActiveCommittee();
		const numShards = systemState.committee.n_shards;
		const { blobId, metadata, primarySlivers, secondarySlivers, rootHash } = (await this.#wasmBindings()).encodeBlob(numShards, blob);
		...
		for (let sliverPairIndex = 0; sliverPairIndex < primarySlivers.length; sliverPairIndex++) {
			...
			const shardIndex = toShardIndex(sliverPairIndex, blobId, numShards);
			const node = await this.#getNodeByShardIndex(committee, shardIndex);
			if (!sliversByNodeMap.has(node.nodeIndex)) sliversByNodeMap.set(node.nodeIndex, {
				primary: [],
				secondary: []
			});
		...
		for (let i = 0; i < systemState.committee.members.length; i++) sliversByNode.push(sliversByNodeMap.get(i) ?? {
			primary: [],
			secondary: []
		});
		return {
	...
	* ```
	*/
	async writeEncodedBlobToNodes({ blobId, metadata, sliversByNode, signal, ...options }) {
		const systemState = await this.systemState();
		const committee = await this.#getActiveCommittee();
		const controller = new AbortController();
		let failures = 0;
		return await Promise.all(sliversByNode.map((slivers, nodeIndex) => {
			return this.writeEncodedBlobToNode({
				...
				if (isAboveValidity(failures, systemState.committee.n_shards)) {
					const error = new NotEnoughBlobConfirmationsError(`Too many failures while writing blob ${blobId} to nodes`);
					controller.abort(error);
					throw error;
				}
				return null;
	...
	*/
	async writeBlobToUploadRelay(options) {
		if (!this.#uploadRelayClient) throw new WalrusClientError("Upload relay not configured");
	...
	* ```
	*/
	async writeEncodedBlobToNode({ nodeIndex, blobId, metadata, slivers, signal, ...options }) {
	...
	* ```
	*/
	async writeBlob({ blob, deletable, epochs, signer, signal, owner, attributes }) {
		if (!this.#uploadRelayConfig) {
			...
			const blobObjectId = (await this.executeRegisterBlobTransaction({
				signer,
				size: blob.length,
				epochs,
				blobId,
			...
			const metadata = await this.computeBlobMetadata({ bytes: blob });
			const blobId = metadata.blobId;
			const transaction = new Transaction();
			transaction.add(this.sendUploadRelayTip({
				size: blob.length,
			...
			await this.#suiClient.core.waitForTransaction({ digest: registerResult.digest });
			const certificate = (await this.writeBlobToUploadRelay({
				blobId,
				blob,
				nonce: metadata.nonce,
	...
	async #executeTransaction(transaction, signer, action) {
		...
		if (result.FailedTransaction) throw new WalrusClientError(`Failed to ${action} (${result.FailedTransaction.digest}): ${result.FailedTransaction.status.error?.message}`);
	...
	async #getCommittee(committee) {
		...
		return {
			byShardIndex,
			nodes: stakingPool.map(({ node_info }, nodeIndex) => {
				...
				for (const shardIndex of shardIndices) byShardIndex.set(shardIndex, node);
				return node;
			})
		};
	}
	...
	*/
	...
	async getFiles({ ids }) {
		...
		for (const id of parsedIds) {
			...
			if (!readersByBlobId.has(blobId)) readersByBlobId.set(blobId, new BlobReader({
				client: this,
				blobId,
				numShards
			}));
			if (id.kind === "quiltPatch") {
				if (!quiltReadersByBlobId.has(blobId)) quiltReadersByBlobId.set(blobId, new QuiltReader({ blob: readersByBlobId.get(blobId) }));
			}
		}
		return parsedIds.map((id) => {
			if (id.kind === "blob") return new WalrusFile({ reader: readersByBlobId.get(id.id) });
			return new WalrusFile({ reader: new QuiltFileReader({
	...
	writeFilesFlow({ files }) {
		const encode = async () => {
			const { quilt, index } = await this.encodeQuilt({ blobs: await Promise.all(files.map(async (file, i) => ({
				contents: await file.bytes(),
				identifier: await file.getIdentifier() ?? `file-${i}`,
				tags: await file.getTags() ?? {}
			}))) });
			return {
				metadata: this.#uploadRelayClient ? await this.computeBlobMetadata({ bytes: quilt }) : await this.encodeBlob(quilt),
				size: quilt.length,
				data: this.#uploadRelayClient ? quilt : void 0,
				index
			};
		};
		const register = ({ data, metadata, index, size }, { epochs, deletable, owner, attributes }) => {
			const transaction = new Transaction();
			...
			transaction.transferObjects([this.registerBlob({
				size,
				epochs,
				blobId: metadata.blobId,
				rootHash: metadata.rootHash,
			...
			if (this.#uploadRelayClient) {
				...
				return {
					...
					certificate: (await this.writeBlobToUploadRelay({
						...
						deletable,
						encodingType: meta$1.metadata.encodingType
					})).certificate
				};
			}
			const meta = metadata;
		...
		const certify = ({ index, metadata, confirmations, certificate, blobObject, deletable }) => {
			return {
				index,
				blobObject,
				metadata,
		...
		async function listFiles({ index, blobObject, metadata }) {
			return index.patches.map((patch) => ({
		...
		function getResults(step, current) {
			if (!stepResults[step]) throw new Error(`${step} must be executed before calling ${current}`);
			return stepResults[step];
		}
		return {
			encode: async () => {
				if (!stepResults.encode) stepResults.encode = await encode();
			},
			register: (options) => {
	...
	writeBlobFlow({ blob }) {
		const encode = async () => {
			return {
				metadata: this.#uploadRelayClient ? await this.computeBlobMetadata({ bytes: blob }) : await this.encodeBlob(blob),
				size: blob.length,
				data: this.#uploadRelayClient ? blob : void 0
			};
		};
		const register = ({ data, metadata, size }, { epochs, deletable, owner, attributes }) => {
			const transaction = new Transaction();
			transaction.setSenderIfNotSet(owner);
			if (this.#uploadRelayClient) {
				const meta = metadata;
		...
		const upload = async ({ data, metadata, deletable }, { digest }) => {
			const blobObject = await this.#getCreatedBlob(digest);
			if (this.#uploadRelayClient) {
				const meta$1 = metadata;
				return {
		...
		const certify = ({ metadata, confirmations, certificate, blobObject, deletable }) => {
			return {
				blobObject,
				metadata,
				transaction: confirmations ? this.certifyBlobTransaction({
		...
		async function getBlob({ blobObject, metadata }) {
			return {
				blobId: metadata.blobId,
				blobObject
			};
